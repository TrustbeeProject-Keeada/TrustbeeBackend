...
export const createJobService = async (data, companyId) => {
...
export const updateJobByIdService = async (jobId, data, companyId) => {
    const job = await prisma.job.findUnique({
        where: { id: jobId },
    });
    if (!job) {
...
export const changeJobStatusService = async (jobId, companyId, status) => {
    const job = await prisma.job.findUnique({
        where: { id: jobId },
    });
    if (!job) {
